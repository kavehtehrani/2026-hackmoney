...
import type { RouteOption } from "@/lib/lifi";
...
export default function PaymentConfirmation({
  ...
  const hasMultipleRoutes = routeOptions && routeOptions.length > 0 && onSelectRoute;
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-5">
        ...
        {/* Route Selection (if multiple routes available) */}
        {hasMultipleRoutes ? (
          <div className="space-y-3">
            ...
            <RouteSelector
              ...
              toTokenSymbol={quote.toToken}
            />
            <div className="text-xs text-muted-foreground">
              {quote.fromChain}
              {quote.fromChainId !== quote.toChainId ? ` → ${quote.toChain}` : ""} •
              Sending {quote.fromAmount} {quote.fromToken}
            </div>
          </div>
        ) : (
          /* Fallback: Single route details from LI.FI */
          <div className="space-y-3">
        ...
        <div className="flex gap-3">
          ...
          <Button
            onClick={onConfirm}
            disabled={isExecuting || (hasMultipleRoutes && !selectedRouteId)}
            className="flex-1"
          >
            {isExecuting ? (
